...
<head>
  ...
  <?php if(!empty( $_GET['teacher_name'])) $page = $_GET['teacher_name'] ?>
  <?php if(empty( $_GET['page_name'])) $link = 'teachers' ?>
...
<?php 
  ...
  if(isset($_POST['addpro'])){
    ...
    $password = $_POST['newactpass'];
       ...
       $target = "../images/teachers/".basename($image);
    ...
    $data=mysqli_query($con,"INSERT INTO teacher (name,classid,phone,email,img,password)VALUES ('$name','$classid','$phone','$email','$image','$password')")or die( mysqli_error($con) );
...
<?php 
  ...
  if(isset($_POST['savepro'])){
    ...
    if (!empty($_FILES['img']['name'])) {
      ...
      $data=mysqli_query($con,"UPDATE teacher SET `img`='$image' where `id`='$id'")or die( mysqli_error($con) );
      ...
      $sql = "UPDATE teacher SET `name` = '$name',`phone` = '$phone',`email` = '$email',`classid` = '$classid'  WHERE  `id`='$id' " ;
...
<?php 
  ...
  if(isset($_POST['block'])){
    ...
    $blockres=$_POST['blockres'];
      ...
      $sql = "UPDATE teacher SET `block` = '1',`blockres` = '$blockres'  WHERE  `id`='$id' " ;
...
<?php 
  ...
  if(isset($_POST['unblock'])){
    ...
    $id=$_POST['unblock'];
      ...
      $sql = "UPDATE teacher SET `block` = '0',`blockres` = ''  WHERE  `id`='$id' " ;
...
<?php 
  ...
  if(isset($_POST['deluser'])){
    ...
    $id=$_POST['deluser'];
      ...
      ($msg=mysqli_error($con));
      if(empty($msg))  $msg="Unblocked";
header("location:teachers"); // Redirecting To Other Page
...
<body onload="showtoast()"  class="page-header-fixed bg-1 sidebar-nav">
  <div class="modal-shiftfix">
    ...
    <?php if (!empty($page)) {
      ...
      $rows =mysqli_query($con,"SELECT * FROM teacher WHERE id ='$page'" ) or die(mysqli_error($con));
    ...
    $classid = $row['classid']; 
        ...
        <div class="col-md-12">
          ...
          <div class="row">
            ...
            <div class="col-md-12 text-center">
              <img src="../images/teachers/<?php echo $actpic ?>" style="width: 200px">
              <br>
              <br>
              <center>
     ...
     <?php
                   ...
                   while($rowx=mysqli_fetch_array($rowsx)){
                    $name= $rowx['name']; 
                    $newcat= $rowx['id']; ?>
 <option <?php if($newcat==$classid) echo 'selected' ?> value = "<?php echo $newcat ?>"><?php echo $name ?></option>
<?php } ?>
...
<div class="row">
  ...
  <button class="btn btn-danger" name="deluser" value="<?php echo $actid ?>">Delete Teacher <i class="fa fa-user-times"></i> </button>
...
<div class="">
  <div class="row">
    ...
    <div class="col-md-12">
      <div class="widget-container fluid-height clearfix overflow">
        <div class="heading">
          <i class="fa fa-teacher"></i>View, Seacrch or Export the teacher.
        </div>
  ...
  while($row=mysqli_fetch_array($rows)){
    ...
    $datec = $row['created_at']; 
        ...
        $blockres = $row['blockres'];
                ...
                <tr style="<?php if($block==1)echo "background: #ff000050"; ?> " class="orow" onclick="window.location='teachers-<?php echo $actid ?>'">
                 ...
                 <td><img src="../images/teachers/<?php echo $actpic ?>" class="minicartimg"></td>
