      ...
      <div class="col-md-6 text-center" style="background: #f1f1f1; padding: 10%;">
        ...
        <?php
           ...
           if (isset($_POST['login'])) {
                       ...
                       $query = mysqli_query($con,"SELECT * from student where password='$password' AND email='$email'")or die(mysqli_error($con));
                       $rows = mysqli_num_rows($query);
                       if ($rows == 1) {



                       $rowsx =mysqli_query($con,"SELECT id FROM student WHERE email='$email'" ) or die(mysqli_error($con));
                   while($rowx=mysqli_fetch_array($rowsx)){
                    $tid= $rowx['id'];
                    }
                       $_SESSION['studentid']=$tid; // Initializing Session
